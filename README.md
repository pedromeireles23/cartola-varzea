...
- Repositório com CI, convenções de build e decisões de arquitetura registradas ([ADR-001](docs/adr/0001-monolito-modular.md), [ADR-004](docs/adr/0004-isolamento-por-organizacao.md), [ADR-008](docs/adr/0008-perfis-de-modalidade-versionados.md) e [ADR-009](docs/adr/0009-importacao-csv-sem-staging.md)).
...
- Pontuação: regra versionada por modalidade e cálculo da rodada no domínio — pontos por evento e posição, banco na mesma posição, capitão, média do técnico e valorização relativa à média da posição —, como função pura das súmulas e da escalação congelada, coberta por testes de ouro com a tabela aprovada. A publicação que grava a apuração vem a seguir.
...
- Fluxos cobertos por testes: 431 de backend (340 unitários, 5 de arquitetura e 86 contra SQL Server real), 181 de frontend e 49 E2E em Playwright desktop/mobile, lendo e-mails reais do Mailpit. Um deles espera o mercado fechar pelo relógio real e vê a escalação congelar na tela do celular.
- Próximo passo: a publicação da rodada, que grava a apuração, o ledger e o preço novo sobre uma revisão imutável das súmulas, e depois a correção de rodada. Google OAuth foi adiado para uma etapa posterior antes da demonstração pública.
