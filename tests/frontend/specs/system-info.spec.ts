...
test('a pagina de sistema mostra dados vindos do banco', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Estado do sistema' })).toBeVisible();
  ...
  const inicializacoes = cartao.getByRole('term').filter({ hasText: 'Inicializações registradas' });
...
test('a navegacao principal funciona pelo teclado', async ({ page }) => {
  ...
  await page.keyboard.press('Tab');
  await expect(page.getByRole('link', { name: 'Cartola Várzea' })).toBeFocused();
});
