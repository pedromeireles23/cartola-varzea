...
public sealed class ImportTemplateTests
{
    ...
    [InlineData(ImportKind.MatchStatistics, "estatisticas-v1.csv")]
    ...
    public void EveryTemplateIsReadBackByTheParserItBelongsTo()
    {
        ...
        var statistics = ImportParser.MatchStatistics(Document(ImportKind.MatchStatistics));

        Assert.Empty(teams.Issues);
        Assert.Empty(athletes.Issues);
        Assert.Empty(coaches.Issues);
        Assert.Empty(matches.Issues);
        Assert.Empty(statistics.Issues);
        ...
        Assert.Equal(ImportTemplates.For(ImportKind.MatchStatistics).SampleRows.Count, statistics.Rows.Count);
    }
    ...
    public void SampleRowsPointAtTeamsThatTheTeamTemplateCreates()
    {
        ...
        // O exemplo de estatísticas usa atletas que o modelo de atletas cria, no time certo.
        var athletes = ImportParser.Athletes(Document(ImportKind.Athletes)).Rows
            .ToDictionary(row => row.SportingName, row => row.TeamName, StringComparer.OrdinalIgnoreCase);
        Assert.All(
            ImportParser.MatchStatistics(Document(ImportKind.MatchStatistics)).Rows,
            row => Assert.Equal(athletes[row.AthleteName], row.TeamName));
    }
